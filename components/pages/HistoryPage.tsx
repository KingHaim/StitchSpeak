    ...
    <>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      role="button"
                      tabIndex={0}
                      onClick={e => { e.stopPropagation(); handleOpenFullView(record); }}
                      onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); handleOpenFullView(record); } }}
                      className="px-3 py-1.5 text-xs font-medium text-brand-600 border border-brand-200 hover:bg-brand-100 rounded-lg transition-colors"
                      aria-label={`Open full view for ${record.fileName}`}
                    >
                      Open full view
                    </span>
                    <span
      ...
      {fullViewRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col">
            <div className="bg-slate-50 p-5 border-b border-slate-100 flex items-start justify-between gap-4">
              <div className="min-w-0">
                <h3 className="text-lg font-bold text-slate-800 truncate">{fullViewRecord.fileName}</h3>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-3 py-2 text-sm font-medium rounded-lg border border-brand-200 text-brand-600 hover:bg-brand-50 transition-colors"
                >
                  Download
                </button>
                <button onClick={handleCloseFullView} className="text-slate-400 hover:text-slate-600 transition" aria-label="Close full view">
                  <CloseIcon className="w-5 h-5" />
                </button>
              </div>
            </div>

            <div className="p-6 overflow-y-auto bg-brand-50/20 flex-1">
              {fullViewHtml ? (
                <div
                  className="bg-white rounded-xl border border-brand-200 p-6 sm:p-8 text-sm sm:text-base text-brand-800 leading-relaxed shadow-sm"
