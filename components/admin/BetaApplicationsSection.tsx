...
export const BetaApplicationsSection: React.FC = () => {
  ...
  const review = async (item: AdminBetaApplication, status: 'approved' | 'rejected') => {
    ...
    try { await reviewAdminBetaApplication(item.id, status); await load(); } catch (e) { setError(e instanceof Error ? e.message : 'Could not update the application.'); } finally { setWorking(null); }
  };
  return <section className="mt-8 overflow-hidden rounded-2xl border border-[#1d2b23]/10 bg-white">
    <div className="flex flex-col gap-5 border-b p-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#617067]">Private beta</p><h2 className="mt-1 font-headline text-2xl font-semibold">Beta applications</h2><p className="mt-1 text-sm text-[#617067]">Review Pattern Rescue applicants and record your decision.</p></div><div className="flex flex-wrap gap-2" role="group" aria-label="Filter beta applications">{filters.map(item => <button key={item.label} type="button" onClick={() => setFilter(item.value)} className={`rounded-lg px-3.5 py-2 text-sm font-bold ${filter === item.value ? 'bg-[#17231d] text-white' : 'bg-[#eef1ef] text-[#425047]'}`}>{item.label}</button>)}</div></div>
