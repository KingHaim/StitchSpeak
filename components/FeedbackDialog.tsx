    ...
    <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="feedback-dialog-title">
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[92vh] flex flex-col">
        <div className="bg-slate-50 p-4 sm:p-6 border-b border-slate-100 flex justify-between items-center">
          <div className="pr-3">
            <h3 id="feedback-dialog-title" className="text-lg font-bold text-slate-800">Send feedback</h3>
            <p className="text-xs text-slate-500">Found a bug or have a question? It goes straight to the team.</p>
          </div>
        ...
        {sent ? (
          <div className="p-6 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-brand-50">
              <span className="material-symbols-outlined text-2xl text-brand-600" aria-hidden>mark_email_read</span>
            </div>
            <p className="text-slate-800 font-semibold mb-1">Thank you!</p>
            <p className="text-sm text-slate-500 mb-5">Your feedback has been sent. We may reply to your account email.</p>
            <button
              type="button"
              onClick={onClose}
              className="w-full py-3 px-4 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-lg shadow-md transition-all"
            >
              Done
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto flex-1">
            ...
            <textarea
              ...
              className="w-full rounded-xl border-2 border-slate-200 focus:border-brand-600 focus:outline-none p-3 text-sm text-slate-800 placeholder:text-slate-400 resize-y"
            />
            <p className="mt-1 mb-4 text-right text-[11px] text-slate-400">{message.length}/{MAX_LENGTH}</p>
            ...
            <button
              ...
              className="w-full flex items-center justify-center py-3.5 px-4 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-lg shadow-md transition-all disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isSending ? (
